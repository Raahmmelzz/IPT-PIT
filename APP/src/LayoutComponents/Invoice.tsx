...
export const Invoice: React.FC<InvoiceProps> = ({ items, customerName, subtotal, onReset }) => {
  const vat = subtotal * 0.12;
  const total = subtotal + vat;
  const invoiceNumber = Math.floor(Math.random() * 900000) + 100000; // Random 6 digit number
  const date = new Date().toLocaleDateString();

  return (
    <div className="bg-white p-8 rounded-2xl shadow-2xl relative w-full">
      {/* This is the printable area.
        Tailwind's 'print:hidden' hides elements when the user clicks 'Save to PDF' 
      */}
      <div className="flex justify-between items-start border-b border-slate-200 pb-6 mb-6">
        <div>
          <h1 className="text-4xl font-black text-slate-800 tracking-tight">INVOICE</h1>
          <p className="text-slate-500 mt-1 font-mono font-medium">#{invoiceNumber}</p>
        </div>
      ...
      {/* Action Buttons (These will NOT show up on the PDF) 
      */}
      <div className="flex gap-4 print:hidden pt-4 border-t border-slate-100">
         ...
         <button 
           ...
           className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-200"
         >
